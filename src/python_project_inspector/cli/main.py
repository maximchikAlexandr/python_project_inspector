"""CLI entry point for python-project-inspector."""
...
from python_project_inspector.core.contracts import AnalysisBatch, ProjectRef, RunMeta, batch_to_json
...
from python_project_inspector.history.worktree import remove_worktree
...
from python_project_inspector.runtime.log import get_logger, set_verbose
from python_project_inspector.runtime.paths import (
...
def _recover_stale_artifacts(ctx: CliContext) -> list[str]:
    ...
    actions: list[str] = []
    locked = project_lock.is_locked(lock_path(ctx.analysis_dir))
    ...
    lock_file = lock_path(ctx.analysis_dir)
...
def _open_store_reader(analysis_dir: Path) -> StoreReader:
    """Open a read-only store reader with schema compatibility errors surfaced."""
    try:
        return StoreReader(store_path(analysis_dir), read_only=True)
...
def _resolve_context(
    ...
    try:
        ...
        for artifact in (analysis, store_path(analysis), worktree_path(analysis)):
...
def analyze(
    ...
    def _run() -> None:
        ...
        try:
            writer = StoreWriter(store_path(ctx.analysis_dir))
        except schema.SchemaIncompatibleError as exc:
            raise click.ClickException(str(exc)) from exc
        if rebuild:
    ...
    try:
        with project_lock.write_lock(lock_path(ctx.analysis_dir)):
            _run()
...
def _run_analyze_loop(
    ...
    click.echo(f"Store: {store_path(ctx.analysis_dir)}")


@cli.command()
@click.option(
    "--metric",
    type=click.Choice(["complexity", "lines", "edges"], case_sensitive=False),
    required=True,
)
...
def query(
    ...
    if project_lock.is_locked(lock_path(ctx.analysis_dir)):
        raise click.ClickException("Analysis in progress; query later.")
    try:
        reader = _open_store_reader(ctx.analysis_dir)
    ...
    try:
        ...
        if metric == "edges":
            ...
            rows = (
                ...
                if metric == "lines"
                else reader.file_complexity_timeseries(mod, rel)
            )
        elif metric == "lines":
            if not module_name:
            ...
            rows = reader.module_lines_timeseries(module_name)
        else:
            if not module_name:
                raise click.ClickException("--module is required for complexity metric")
            if not reader.module_exists(module_name):
                raise click.ClickException(f"Unknown module: {module_name}")
            rows = reader.module_complexity_timeseries(module_name)
    finally:
        reader.close()
    _emit_query_rows(rows, output_format)
...
def serve(ctx: CliContext, host: str, port: int, open_browser: bool) -> None:
    ...
    store_file = store_path(ctx.analysis_dir)
    if store_file.is_file():
        try:
            ...
            raise
    from python_project_inspector.server.app import STATIC_DIR, _static_dir, create_app
    import uvicorn
    ...
    app = create_app(store_path(ctx.analysis_dir), lock_path(ctx.analysis_dir))
...
def doctor(ctx: CliContext, recover_stale: bool) -> None:
    ...
    if store_path(ctx.analysis_dir).is_file():
        try:
            reader = _open_store_reader(ctx.analysis_dir)
