...
type Props = {
  ...
  onSectionsExpandedChange: (patch: Partial<Record<GraphSectionKey, boolean>>) => void;
  onResetForces: () => void;
  onResetAll: () => void;
  onZoom: (kind: ZoomCommandKind) => void;
  onLayout: (kind: LayoutCommandKind) => void;
  onClearFocus: () => void;
  onPinSelected: () => void;
  stats: GraphStats;
  ...
  commitPositionLabel: string;
  timelapse: { playing: boolean; speed: number };
  onTimelapse: (action: TimelapseAction) => void;
  collapsed: boolean;
